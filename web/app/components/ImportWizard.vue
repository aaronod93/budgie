...
<template>
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
    <div class="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-sm bg-paper-200 p-6 text-ink-800 shadow-xl">
      ...
      <div v-if="result" class="space-y-4">
        ...
        <div class="flex justify-end">
          <button class=" rounded-sm bg-accent-400 px-4 py-2 font-medium text-ink-900 hover:bg-accent-500" @click="emit('close')">Done</button>
        </div>
      </div>
      ...
      <template v-else>
        ...
        <template v-if="rows.length">
          <div class="mb-4 grid grid-cols-2 gap-3 md:grid-cols-3">
            ...
            <label class="text-sm">Date format
              <select v-model="dateFormat" class="mt-1 w-full rounded-sm border border-paper-400 bg-white px-2 py-1">
            ...
            <label class="text-sm">Amount layout
              <select v-model="amountMode" class="mt-1 w-full rounded-sm border border-paper-400 bg-white px-2 py-1">
            ...
            <label class="text-sm">Date column
              <select v-model.number="dateCol" class="mt-1 w-full rounded-sm border border-paper-400 bg-white px-2 py-1">
                <option v-for="(name, i) in columns" :key="i" :value="i">{{ name }}</option>
              </select>
            </label>
            <label class="text-sm">Payee column
              <select v-model.number="payeeCol" class="mt-1 w-full rounded-sm border border-paper-400 bg-white px-2 py-1">
                <option :value="-1">—</option>
                <option v-for="(name, i) in columns" :key="i" :value="i">{{ name }}</option>
              </select>
            </label>
            <label class="text-sm">Memo column
              <select v-model.number="memoCol" class="mt-1 w-full rounded-sm border border-paper-400 bg-white px-2 py-1">
                <option :value="-1">—</option>
                <option v-for="(name, i) in columns" :key="i" :value="i">{{ name }}</option>
              </select>
            </label>
            <label v-if="amountMode === 'single'" class="text-sm">Amount column
              <select v-model.number="amountCol" class="mt-1 w-full rounded-sm border border-paper-400 bg-white px-2 py-1">
                <option v-for="(name, i) in columns" :key="i" :value="i">{{ name }}</option>
              </select>
            </label>
            <template v-else>
              <label class="text-sm">Outflow column
                <select v-model.number="outflowCol" class="mt-1 w-full rounded-sm border border-paper-400 bg-white px-2 py-1">
                  <option v-for="(name, i) in columns" :key="i" :value="i">{{ name }}</option>
                </select>
              </label>
              <label class="text-sm">Inflow column
                <select v-model.number="inflowCol" class="mt-1 w-full rounded-sm border border-paper-400 bg-white px-2 py-1">
                  <option v-for="(name, i) in columns" :key="i" :value="i">{{ name }}</option>
                </select>
              </label>
            </template>
          ...
          <div class="mb-4 overflow-x-auto rounded-sm border border-paper-300">
            <table class="w-full text-sm">
              <tbody>
                <tr v-for="(row, i) in preview" :key="i" class="border-b border-paper-300 last:border-0">
                  <td class="px-3 py-1.5 text-ink-700">{{ row.date }}</td>
                  <td class="px-3 py-1.5">{{ row.payee_name ?? '—' }}</td>
                  <td class="px-3 py-1.5 text-mist-700">{{ row.memo }}</td>
        ...
        <div class="flex justify-end gap-2">
          ...
          <button
            ...
            class=" rounded-sm bg-accent-400 px-4 py-2 font-medium text-ink-900 hover:bg-accent-500 disabled:opacity-50"
            @click="submit"
          >
